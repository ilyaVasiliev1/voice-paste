...
final class ReadinessModelStateObservationTests: XCTestCase {
  ...
  func test_publishedProjectedValue_emitsBeforeThePropertyIsUpdated() async throws {
    let directory = try makeEmptyModelDirectory()
  ...
  private func makeEmptyModelDirectory() throws -> URL {
    let directory = FileManager.default.temporaryDirectory
      .appendingPathComponent("ReadinessModelTests-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    return directory
  }
