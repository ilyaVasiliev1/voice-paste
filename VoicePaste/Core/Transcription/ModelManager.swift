...
public final class ModelManager: ObservableObject {
  ...
  private let makeTranscriber:
    (URL, String, @escaping @Sendable (Double) -> Void) async throws -> Transcribing
  ...
  /// Production uses sixty seconds per configured minute. Tests inject a
  /// shorter unit so timer and cancellation behavior stay real without a
  /// minute-long wall-clock wait.
  private let unloadNanosecondsPerMinute: UInt64
  ...
  public init(
    modelDirectory: URL,
    makeTranscriber:
      @escaping (URL, String, @escaping @Sendable (Double) -> Void) async throws -> Transcribing =
      ModelManager.defaultTranscriberFactory,
    ...
    self.unloadNanosecondsPerMinute = unloadNanosecondsPerMinute
  ...
  private func waitForInitialModelDiscovery() async {
    guard let task = initialModelDiscoveryTask,
      let token = initialModelDiscoveryToken
    else { return }
  ...
  public func checkStorage() -> ModelError? {
    ...
    if available < ModelCatalog.approximateSizeBytes {
      return .insufficientStorage(
        requiredBytes: ModelCatalog.approximateSizeBytes, availableBytes: available)
    }
    return nil
  }
  ...
  private func performLoad(generation: UInt64, allowNetworkInstall: Bool) async throws
    -> Transcribing
  {
    ...
    do {
      ...
      Task {
        await DiagnosticLog.shared.log("model.load.failed", detail: String(describing: error))
      }
      ...
      guard hadExistingLocalModel,
        ...
        let corruptFolder = LocalModelDetection.discoverModelFolder(in: modelDirectory)
      else {
      ...
      Task {
        await DiagnosticLog.shared.log("model.local.invalid", detail: String(describing: error))
      }
  ...
  private func loadWithAutoRetry(
    ...
    for attempt in 0...retryCount {
      do {
        ...
        Task {
          await DiagnosticLog.shared.log("model.load.retry", detail: "attempt \(attempt + 1)")
        }
    ...
    if source.usesDirectArchive {
      if needsModel || needsTokenizer {
        try await downloadFromGitHub(
          model: needsModel, tokenizer: needsTokenizer, generation: generation)
      }
    ...
    do {
      ...
      Task {
        await DiagnosticLog.shared.log(
          "model.github.download.failed", detail: String(describing: error))
      }
      throw error
    }
  }
  ...
  private func handleDownloadProgress(fraction rawFraction: Double, generation: UInt64) {
    ...
    if let last = lastByteSample {
      ...
      if elapsedSeconds > 0, deltaBytes >= 0 {
        ...
        smoothedSpeedBytesPerSecond =
          smoothedSpeedBytesPerSecond.map {
    ...
    let hasEnoughSpeedSignal: Bool = {
      guard speedSampleCount >= Self.minSpeedSamplesForETA,
        ...
        let speed = smoothedSpeedBytesPerSecond, speed > 0
      else { return false }
  ...
  public func endTask(unloadMinutes: Int) {
    ...
    let (nanoseconds, overflow) = UInt64(unloadMinutes).multipliedReportingOverflow(
      by: unloadNanosecondsPerMinute
    )
  ...
  public func unloadNow() {
    ...
    if wasResident {
      Task {
        await DiagnosticLog.shared.log(
          "model.unload", detail: "from=\(Self.describe(previousState))")
      }
    }
  }
