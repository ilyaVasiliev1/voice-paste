...
### Requirement: Выделенный фрагмент переводится на устройстве
<!-- id: LectureView.translateSelection -->
<!-- entities: LectureView -->
<!-- platforms: macos -->
<!-- enforced_macos: VoicePaste/UI/Lecture/LectureView.swift -->

Выделенный в расшифровке текст SHALL переводиться системным переводчиком
macOS, работающим на устройстве. Перевод не отправляет текст наружу — иначе
обещание офлайна нарушено.

#### Scenario: перевод выделенного
- **WHEN** в расшифровке выделен фрагмент и вызван перевод
- **THEN** показывается перевод, исходный текст остаётся на месте
