...
### Requirement: Установка убирает недействительные записи универсального доступа
<!-- id: install-local.resetAccessibility -->
<!-- entities: VoicePasteApp -->
<!-- platforms: macos -->
<!-- enforced_macos: scripts/install-local.sh -->

Перед установкой новой копии система SHALL очищать выданный ей ранее
универсальный доступ.

Причина в том, как macOS привязывает разрешение. Оно относится не к
приложению вообще, а к требованию его подписи, и у сборки без сертификата это
требование — хеш конкретного двоичного файла. Каждая установка даёт новый хеш,
и прежняя запись к новой копии не относится: приложение не видит выданного
доступа, а перевыдача не помогает, потому что запись в списке говорит о
прежнем хеше. Записи при этом копятся — по одной на установку, и ни одна не
работает.

Очистка делает состояние однозначным: список пуст, доступ выдаётся один раз
через системные настройки и работает до следующей установки.

#### Scenario: установка поверх прежней копии
- **WHEN** ставится новая сборка
- **THEN** прежняя запись доступа убрана, а человеку сказано выдать доступ заново
